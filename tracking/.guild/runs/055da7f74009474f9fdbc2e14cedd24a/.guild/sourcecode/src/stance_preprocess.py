...
def df_stance_to_nli(df, stances = ['AGAINST', 'FAVOR', 'NONE'], is_test_split=False):
    ...
    for i, e in df.iterrows():
        if is_test_split: # query the 3 options, labels is kept as the stance NOT as nli            
            ...
            # only considering polarizing entailments and having a NEUTRAL entailment in case of NO stance
            # it generates 2 examples out of each entry
            for stance in ['AGAINST', 'FAVOR']:
                e['Claim2'] = f"{stance_nli_prefix[stance]}{e['Claim']}."
                e['labels'] = nli_labels['NEUTRAL'] if e['Stance']=='NONE' else (nli_labels['ENTAILMENT'] if e['Stance'] == stance else nli_labels['CONTRADICTION'])
                tmp = tmp.append(e)            

...
# export PYTHONPATH=brat/tools:$PYTHONPATH
# sys.path.insert(1, '/path/to/application/app/folder')
sys.path.insert(1, 'brat/tools')
from brat.tools.sentencesplit import sentencebreaks_to_newlines
...
def argparser():
    ...
    ap.add_argument('-o', '--outsuffix', default="iob",
                    help='Suffix to add to output files (default "iob")')
    ap.add_argument('-v', '--verbose', default=False, action='store_true',
                    help='Verbose output')
    ap.add_argument('-i', '--input-dir',  default=None, action='append', 
                    help='input directory where are stored the ann and text files')
    ap.add_argument('-d', '--output-dir', default='.',
                    help='output directory where will be stored the IOB files')
    ap.add_argument('-t', '--text', default=None, action='append', nargs='+',
    ...
    for i, l in enumerate(lines):
        ...
        for o in range(start, end):
            if o in offset_label:
                if o != start:
                    print('\nWarning: annotation-token boundary mismatch: "%s" --- "%s"\n' % (
        ...
        for fn in files:
            ...
            try:
                ...
                if fn == '-' or not options.outsuffix:
                    ...
                    ofn = options.output_dir + '/' + os.path.basename(path.splitext(fn)[0]) + options.outsuffix
...
def main(argv=None):
    ...
    if options.text:
        ...
        print(options.text, options.output_dir)
        for filename in options.text:
            process_files(filename)
    elif options.input_dir:
        print(type(options.input_dir))
        print(options.input_dir, options.output_dir)
        for dirname in options.input_dir:
            print(glob.glob(dirname + '/*.txt'))
            process_files(glob.glob(dirname + '/*.txt'))

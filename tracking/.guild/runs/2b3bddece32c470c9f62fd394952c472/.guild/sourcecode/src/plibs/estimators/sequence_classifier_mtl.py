...
class SequenceClassifierMTL(LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        for task in self.tasks:
            ...
            # loggerx.debug(f"--> yhat:: {type(yhat)} :: {yhat.device} :: {type(yhat.device)}")
            # loggerx.debug(f"--> ytrue:: {type(ytrue)} :: {ytrue.device} :: {type(ytrue.device)}")
            # loggerx.debug(f"--> metrics:: {type(self.valid_metrics[task])} :: {type(self.valid_metrics[task])}")

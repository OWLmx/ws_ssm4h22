...
from plibs.estimators.crf_seqreclassifier import Reclassifier, reclasify_event_sequences


logger = logging.getLogger(__name__)


def predict(tgt_data:pd.DataFrame, model_path:str, model_type:str, input_feats:list, label_mapper=None, rs_prefix="reclsevent", output_logits=False, **kwargs):
    ...
    predictor = Reclassifier(num_labels=len(input_feats))
    predictor.load_state_dict(torch.load(model_path))

    # test the trained model
    logger.info(f"\n=========  Infering on [{tgt_data.shape}] using features: [{input_feats}]  ==========")    
    _, reclassifications = reclasify_event_sequences(tgt_data, 
    current_yhat_column="evvani1_yhat_label", 
    new_yhat_column=f'{rs_prefix}yhat', 
    reclassifier=predictor, input_feats=input_feats)
    ...
    logger.info(f"Reclassifications: \n\t {reclassifications}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_features', type=str, default="evvani1_logits_0,evvani1_logits_1,evvani1_logits_2", help="Features in the data that will be passed as input to the model") 

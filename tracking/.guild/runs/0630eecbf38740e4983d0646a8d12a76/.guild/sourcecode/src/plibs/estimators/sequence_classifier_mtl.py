...
class SequenceClassifierMTL(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.tasks = {'task': num_labels} if type(num_labels) == int or type(num_labels) == float or num_labels.isnumeric() else {num_labels.split('|')[0]: int(num_labels.split('|')[1]) for num_labels in num_labels.split(',')}

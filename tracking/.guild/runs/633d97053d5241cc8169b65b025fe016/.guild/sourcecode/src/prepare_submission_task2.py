...
def prepare_submission_2b(df_predictions, original_data, yhat_field = 'yhat', **kwargs):

    df = df_predictions[['id', 'text', 'claim', yhat_field]]
    print(df)
    # val = val.drop(columns=['labels', 'Premise']).rename(columns={'predictions': 'Stance'})
    df = df.rename(columns={'claim': 'Claim', yhat_field: 'Premise'})
    print(df)
    if original_data:
        # check consistency
        df_original = pd.read_csv(original_data, sep='\t')
    ...
    # expected cols: id	text	Claim	Stance              
    df.to_csv('premise_predictions.tsv', sep='\t', index=False)

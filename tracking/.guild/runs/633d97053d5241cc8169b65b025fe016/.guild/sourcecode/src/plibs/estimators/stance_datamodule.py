...
class StanceDataModule(LightningDataModule):
    ...
    task_text_field_map = {
        ...
        "sentclaim_sentiment": ["Claim2", "tweet_text_clean"],
        "covidlies": ["misconception", "tweet_text_clean"],
        "joint_sentclaim_premise": ["Claim2", "tweet_text_clean"],
        "joint_sentclaim_premise_extrasentiment": ["Claim2", "tweet_text_clean"],
        "stance_as_nli": ["tweet_text_clean","Claim2"],
    }

    task_label_field_map = {
        "base": ["labels"],
        "sentclaim": ["labels"],
        "sentclaim_sentiment": ["labels"],
        "covidlies": ["labels"],
        "joint_sentclaim_premise": ["labels", "Premise"],
        "joint_sentclaim_premise_extrasentiment": ["labels", "Premise"],
        "stance_as_nli": ["labels"],
    }    
    ...
    task_extrafeat_num = {
        "sentclaim_sentiment": ["logits_0", "logits_1", "logits_2"],
        "joint_sentclaim_premise_extrasentiment": ["logits_0", "logits_1", "logits_2"],
    }

    task_num_labels = {
        "base": 3, # 'AGAINST', 'FAVOR', 'NONE'
        "sentclaim": 3,
        "sentclaim_sentiment": 3,
        "covidlies": 3,
        "joint_sentclaim_premise": "labels|3,Premise|2",
        "joint_sentclaim_premise_extrasentiment": "labels|3,Premise|2",
        "stance_as_nli": 3,
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        tokenizer_type = kwargs.get('tokenizer_type', None)
        self.tokenizer = kwargs.get("tokenizer", AutoTokenizer.from_pretrained(tokenizer_type if tokenizer_type and tokenizer_type!='self' else self.model_name_or_path, use_fast=True))
        ...
        self.subsample = 1.0 if subsample is None else subsample # subsample training data
        ...
        if "label_encoder" in kwargs: #.get("label_encoder", None): #TODO: handle labelencoders for multi-target (joint learning)
        ...
        if len(self.label_fields)>1: # for multi-target learning 
    ...
    def read_dataset(self, path: str, as_dataframe=True, sample_size=-1):
        ...
        df = df if sample_size <=0  else df.sample(n=sample_size) if float(sample_size.is_integer()) else df.sample(frac=float(sample_size)) # now there are columns with Nones
    ...
    def setup(self, stage: str):
        ...
        for split in self.dataset.keys():
            ...
            self.columns = [c for c in self.dataset[split].column_names if c in self.loader_columns or (c.startswith('exf_c_') or c.startswith('exf_n_') or c.startswith('labels_')) ]
    ...
    def train_dataloader(self):
        if self.stratified_batch_sampling and len(self.label_fields)==1: # not possible in Joint Learning (predict multiple values)
    ...
    def convert_to_features(self, example_batch, indices=None):
        ...
        for label_field in self.label_fields: 
            # features[f"labels_{label_field}" if len(self.label_fields) > 1 else "labels"] = example_batch[label_field]
            features[f"labels_{label_field}" if len(self.label_fields) > 1 else "labels"] = list(map(int, example_batch[label_field]))
        ...
        for f in self.extrafeat_num:
            ...
            features[f"exf_n_{f}"] = example_batch[f]
        ...
        # logger.debug(f"DATA moduel \t --> {features}")
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--tokenizer_type", type=str, default="", help="Tokenizer type for loading fine-tuned models ")
        parser.add_argument("--label_encoder", type=str, default="")
        parser.add_argument("--subsample",default=1.0,type=float,help="To only use a randomply picked fraction of training data ")        
